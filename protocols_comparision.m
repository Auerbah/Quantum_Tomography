...
x1 = X_Tetrahedron;% Выбранные протоколы
x2 = X_Cube;
x3 = X_Octahedron;
...
[m1, s1] = size(x1);
t1 = time_for_protocol(n, s1, m1, 'uniform');

[m2, s2] = size(x2);
t2 = time_for_protocol(n, s2, m2, 'uniform');

[m3, s3] = size(x3);
t3 = time_for_protocol(n, s3, m3, 'uniform');

%% 

[dF_average_teor_1, d_1] = dF_mixed_with_noise_new(x1, dm, t1, T, T1, T2, flag_a_r, flag_p_r, n);
fprintf("dF_average_teor_1 = %f\n", dF_average_teor_1)

[dF_average_teor_2, d_2] = dF_mixed_with_noise_new(x2, dm, t2, T, T1, T2, flag_a_r, flag_p_r, n);
fprintf("dF_average_teor_2 = %f\n", dF_average_teor_2)

[dF_average_teor_3, d_3] = dF_mixed_with_noise_new(x3, dm, t3, T, T1, T2, flag_a_r, flag_p_r, n);
...
% Create plot
plot(x_p,p_1,'DisplayName','Протокол тетраэдра 4 строки','HitTest','off','LineWidth',10,...
